...
WHERE rml.objid = $P{remittanceid} 
  AND rl.voided = 0   
GROUP BY rl.afid, CASE WHEN af.aftype = 'nonserial' THEN ri.fundname ELSE CASE WHEN ia.groupid IS NULL THEN ri.fundname ELSE CONCAT(ia.fundname, ia.groupid) END  END  
...
SELECT  
	concat(u.lastname,', ', u.firstname) AS particulars,  
	SUM(ri.amount) AS amount  
...
WHERE rem.objid = $P{objid}
  AND r.doctype = 'RPT'  
GROUP BY rp.period, r.txndate, rl.taxpayername, rl.tdno, r.serialno, rl.municityname, rl.barangay, rl.classcode    
