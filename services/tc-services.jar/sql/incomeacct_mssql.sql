...
[getList]
SELECT * FROM incomeaccount ${filters} ORDER BY accttitle   


[getIncomeAccountList]
SELECT objid, acctno, acctcode, accttitle, fundid, fundname, defaultvalue, amounttype, allowdiscount  
FROM incomeaccount 
...
[getNGASData]
SELECT 
	( COALESCE(p2.acctcode, '') + ' - ' + COALESCE(p2.accttitle,'UNMAPPED') ) as parent2, 
	( COALESCE(p1.acctcode, '') + ' - ' + COALESCE(p1.accttitle,'UNMAPPED') ) as parent1, 
	( COALESCE(p.acctcode, '') + ' - ' + COALESCE(p.accttitle,'UNMAPPED') ) as parent, 
	( COALESCE(ic.acctno, '') + ' - ' + COALESCE(ic.accttitle,'UNMAPPED') ) as account 
